document.addEventListener("DOMContentLoaded", (event) => {
  const botaoAdicionarMateria = document.querySelector(
    "#botao-adicionar-materia"
  );
  const botaoAdicionarMateriaCorpo = document.querySelector(
    "#botao-adicionar-materia-corpo"
  );
  ...
  const logo = document.getElementById("logo-mentora");
  ...
  botaoAdicionarMateriaCorpo.onclick = function () {
    //não pode usar o mesmo id para o mesmo botão, não le ids iguais.
    formulario.style.display = "block";
    fundo.style.display = "block";
    inputMateria.focus();
  };
  ...
  if (!localStorage.getItem("contadorMaterias")) {
    //se não existir contador (ID)
    localStorage.setItem("contadorMaterias", "1"); //cria o contador começando em 1
  }

  //contador de IDs para as atividade
  if (!localStorage.getItem("contadorAtividades")) {
    //se não existir contador (ID)
  ...
  if (!localStorage.getItem("listaMaterias")) {
    //se não existir lista de materias
  ...
  function salvarMateria() {
    ...
    const contadorAtual = parseInt(
      localStorage.getItem("contadorMaterias"),
      10
    ); // pega contador atual
  ...
  function removerMateria(id) {
    ...
    const novaLista = lista.filter(function (m) {
      return m.id !== id;
  ...
  function carregarMateriasNaTela() {
    ...
    lista.forEach(function (materia) {
      ...
      // Se a atividade estava salva como concluída no localStorage,
      // aplica a classe automaticamente ao carregar a lista
  ...
  function atualizarSelectMaterias() {
    ...
    const listaMateriasSelect = JSON.parse(
      localStorage.getItem("listaMaterias") || "[]"
    );
  ...
  const adicionarAtividade = document.getElementById(
    "botao-adicionar-atividade-corpo"
  );
  const adicionarAtividadeIcone = document.getElementById(
    "adicionar-atividade"
  );

  const formularioAtividade = document.getElementById(
    "sobrepor-formulario-atividade"
  );

  const botaoSalvarAtividade = document.getElementById(
    "botao-salvar-atividade"
  );
  const botaoCancelarAtividade = document.getElementById(
  ...
  function salvarAtividade() {
    ...
    const materiaId =
      materiaSelecionada === "0" ? null : Number(materiaSelecionada);

    // pegar data AGORA
    const dataFim = inputDataAtividade.value;

    //status da atividade, por enquanto ainda nao foi feita pois esta sendo adicionada agora
    const status = false;
    ...
    const contadorAtual = parseInt(
      localStorage.getItem("contadorAtividades"),
      10
    );
  ...
  function carregarAtividadesNaTela() {
    ...
    lista.forEach(function (atividade) {
      ...
      if (atividade.concluida) {
        botaoExcluir.disabled = true; // Desativa o clique
        botaoExcluir.style.opacity = "0.4"; // Diminui a opacidade (indicador visual)
        botaoExcluir.style.cursor = "default"; // Remove a mãozinha do cursor
      }
      ...
      // Se a atividade estava concluída, marca o botão também
      if (atividade.concluida === true) {
        botaoConcluir.classList.add("concluida");
      }
      ...
      botaoConcluir.onclick = function () {
        // O estado de 'concluida' é o oposto do estado atual da atividade no momento do clique.
        const isConcluida = !atividade.concluida;

        // 1. Atualiza no localStorage
        const listaAtividadesStorage = JSON.parse(
          localStorage.getItem("listaAtividades") || "[]"
        );
        const index = listaAtividadesStorage.findIndex(
          (a) => a.id === atividade.id
        );
        if (index !== -1) {
          listaAtividadesStorage[index].concluida = isConcluida; // Salva o novo estado
          localStorage.setItem(
            "listaAtividades",
            JSON.stringify(listaAtividadesStorage)
          );
        }

        // 2. Re-renderiza toda a lista. Isso garante:
        // - Que o botão de excluir seja reativado/desativado (Correção da Falha 1)
        // - Que a atividade seja reordenada (se for o caso)
        // - Que os estilos 'concluida' sejam aplicados/removidos (Correção da Falha 2)
        carregarAtividadesNaTela();

        // Nenhuma lógica de toggle visual é necessária aqui, pois a re-renderização faz isso.
      };
    ...
    reloadCalendarEvents();
    atualizarGraficoAtividades();
  }

    // ======== Função: atualiza o gráfico de Atividades (Concluídas x Não) ========
    // Colocar dentro do mesmo escopo do DOMContentLoaded (ou seja, no script.js onde
    // suas outras funções existem). Substitui leituras por 'atividades' pela
    // chave que você usa: 'listaAtividades'.
    function atualizarGraficoAtividades() {
        // Pega a lista de atividades que você já salva no localStorage
        const lista = JSON.parse(localStorage.getItem('listaAtividades') || '[]');

        // Contagem de concluídas
        const concluidas = lista.filter(a => a.concluida === true).length;

        // Contagem de não concluídas
        const naoConcluidas = lista.length - concluidas;

        // Se o elemento canvas nao existir, aborta (evita erro)
        const canvas = document.getElementById('graficoAtividades');
        if (!canvas) return;

        // Se já tem um gráfico anterior, destrói para recriar (evita sobreposição)
        if (window.graficoAtividades instanceof Chart) {
            window.graficoAtividades.destroy();
        }

        // Cria o gráfico tipo "doughnut" (rosquinha)
        window.graficoAtividades = new Chart(canvas, {
            type: 'doughnut',
            data: {
                labels: ['Concluídas', 'Não concluídas'],
                datasets: [{
                    data: [concluidas, naoConcluidas],
                    backgroundColor: ['#58c38c', '#f36b6b'] // verde / vermelho
                }]
            },
            options: {
                responsive: true,
                ...
                cutout: '70%',
                plugins: {
                    legend: { display: false }, // esconde legenda (opcional)
  ...
  function alternarPainel(idPainelAlvo) {
    // 1. Definir a lista de IDs de todos os painéis
    const idsPaineis = [
      "painel-de-progresso",
    ...
    const painelAlvo = document.getElementById(idPainelAlvo);

    const estaExpandido =
      painelAlvo && painelAlvo.style.width === "calc(100vw - 60px)";

    if (estaExpandido) {
      idsPaineis.forEach((id) => {
    ...
    idsPaineis.forEach((id) => {
      ...
      if (painel) {
        if (id === idPainelAlvo) {
          if (idPainelAlvo != "calendario-e-prazos") {
            const ulatividades = document.getElementById("lista-atividades");
            ulatividades.style.display = "grid";
            ulatividades.style.gridTemplateColumns = "repeat(3, 1fr)";
            ulatividades.style.gap = "2%";
            // painel.style.alignItems = "start";
          } else {
            painel.style.display = "block";
          }

          painel.style.width = "calc(100vw - 60px)";
        } else {
          painel.style.display = "none";
        }
      }
  ...
  // Adiciona o Event Listener para o clique duplo nos títulos dos painéis
  const titulosPaineis = document.querySelectorAll(".painel-titulo");
  titulosPaineis.forEach((titulo) => {
    titulo.addEventListener("dblclick", (event) => {
      const painelDiv = event.currentTarget.closest("div[id]");
      if (painelDiv) {
        alternarPainel(painelDiv.id);
      }
    });
  });
  ...
  calendar = new FullCalendar.Calendar(
    document.getElementById("calendar-fullscreen"),
    {
  ...
  // =======================================================
  // NOVO: OBSERVER PARA REDIMENSIONAR O CALENDÁRIO
  // =======================================================

  // Função para atualizar o tamanho do calendário
  function atualizarTamanhoCalendario() {
    if (calendar) {
      calendar.updateSize();
    }
  }

  // Observa o painel do calendário para redimensionar o FullCalendar quando o painel for arrastado.
  const painelCalendario = document.getElementById("calendario-e-prazos");

  if (painelCalendario) {
    // Para redimensionar dinamicamente quando o usuário arrasta o painel:
    // Dispara a atualização a cada 200ms enquanto o painel estiver sendo redimensionado
    let resizeTimer;
    painelCalendario.addEventListener("mouseup", () => {
      clearTimeout(resizeTimer);
      resizeTimer = setTimeout(atualizarTamanhoCalendario, 200);
    });

    // NOVO: Quando o painel muda de visibilidade (expansão), também deve atualizar o tamanho
    // Isso é útil após o duplo clique (função alternarPainel)
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.attributeName === "style") {
          atualizarTamanhoCalendario();
        }
      });
    });
  ...
  function pesquisar(termo) {
    const atividades =
      JSON.parse(localStorage.getItem("listaAtividades")) || [];
    const materias = JSON.parse(localStorage.getItem("listaMaterias")) || [];

    const filtradasAtividades = atividades.filter((a) =>
      a.nome.toLowerCase().includes(termo)
    );
    const filtradasMaterias = materias.filter((m) =>
      m.nome.toLowerCase().includes(termo)
    );

    carregarAtividadesFiltradas(filtradasAtividades);
    carregarMateriasFiltradas(filtradasMaterias);
  }

  function carregarAtividadesFiltradas(lista) {
    listaAtividades.innerHTML = "";
    ...
    lista.forEach(function (atividade) {
      ...
      // botão excluir
      const botaoExcluir = document.createElement("button");
      botaoExcluir.classList.add("botao-excluir", "botao-acao-vertical");
      ...
      if (atividade.concluida) {
        ...
        botaoExcluir.style.cursor = "default";
      }
      ...
      // botão concluir
      const botaoConcluir = document.createElement("button");
      botaoConcluir.classList.add("botao-concluir", "botao-acao-vertical");
      ...
      const iconeConcluir = document.createElement("img");
      iconeConcluir.src = "utilitarios/checked.png";
      botaoConcluir.appendChild(iconeConcluir);

      botaoConcluir.onclick = function () {
        const atividadesStorage =
          JSON.parse(localStorage.getItem("listaAtividades")) || [];
        const index = atividadesStorage.findIndex((a) => a.id === atividade.id);

        if (index !== -1) {
          atividadesStorage[index].concluida = !atividade.concluida;
          localStorage.setItem(
            "listaAtividades",
            JSON.stringify(atividadesStorage)
          );
        }

        carregarAtividadesNaTela(); // recarrega a lista completa
      };
  ...
  function carregarMateriasFiltradas(lista) {
    listaMaterias.innerHTML = "";
...
function atualizarGraficoAtividades() {
    ...
    window.graficoAtividades = new Chart(canvas, {
        ...
        data: {
            ...
            datasets: [{
                data: [concluidas, naoConcluidas],
                backgroundColor: ['#4CAF50', '#BDBDBD'] // <-- TROQUE AS CORES AQUI
            }]
        },
        options: {
            responsive: true,
    ...
    const texto = document.getElementById("texto-grafico");
    texto.textContent = `Concluídas ${concluidas}/${lista.length}`;
}

// ============================
// CHAMA O GRÁFICO AO INICIAR
// ============================
atualizarGraficoAtividades();
